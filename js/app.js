...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('st-darkmode') === 'true') document.documentElement.setAttribute('data-theme', 'dark');
    ...
    loadProgress() {
        try { const s = localStorage.getItem('st-progress'); if (s) this.progress = JSON.parse(s);
            const sq = localStorage.getItem('st-quiz-results'); if (sq) this.quizResults = JSON.parse(sq);
        } catch (e) { this.progress = {}; this.quizResults = {}; }
    },
    saveProgress() { localStorage.setItem('st-progress', JSON.stringify(this.progress)); localStorage.setItem('st-quiz-results', JSON.stringify(this.quizResults)); },
    ...
    showDashboard() {
        ...
        let html = `<div class="fade-in"><div class="dashboard-hero"><h2>雑談アカデミーへようこそ</h2>
            <p>「何を話せばいいかわからない…」を解消！天気、ニュース、趣味から、場を和ませる雑談力まで体系的に学べるプラットフォームです。</p></div>
    ...
    showCompletionMessage() {
        document.getElementById('modalContent').innerHTML = `<h2>🎓 おめでとうございます！</h2>
            ...
            <p>全モジュール完了！<br>雑談力が大幅にアップしました。<br>さあ、実践で試してみましょう！</p>
    ...
    toggleDarkMode() { const d = document.documentElement.getAttribute('data-theme') === 'dark'; if (d) { document.documentElement.removeAttribute('data-theme'); localStorage.setItem('st-darkmode','false'); } else { document.documentElement.setAttribute('data-theme','dark'); localStorage.setItem('st-darkmode','true'); } },
    resetProgress() { if (confirm('全ての学習進捗をリセットしますか？')) { this.progress = {}; this.quizResults = {}; localStorage.removeItem('st-progress'); localStorage.removeItem('st-quiz-results'); this.buildSidebar(); this.updateGlobalProgress(); this.showDashboard(); } },
    showReference() {
        this.showView('referenceView'); this.buildSidebar();
        const c = document.getElementById('referenceView');
        const sections = [
            { category: '雑談の基本', terms: [
                { name: 'アイスブレイク', desc: '初対面の緊張を和らげるための軽い会話や活動' },
                { name: 'スモールトーク', desc: '天気や時事など軽い話題でのカジュアルな会話' },
                { name: 'ラポール', desc: '相互信頼に基づいた親しみのある関係性' },
                { name: 'ペーシング', desc: '相手の話し方やテンポに合わせるテクニック' },
            ]},
            { category: '話題・ネタ', terms: [
                { name: '木戸に立てかけし衣食住', desc: '雑談ネタの頭文字（季節・道楽・ニュース・旅・天気・家族・健康・仕事・衣・食・住）' },
                { name: 'たちつてと', desc: '雑談テーマ（食べ物・地域・通勤通学・天気・友人知人）' },
                { name: '適度に整理すべし', desc: '雑談NGネタ回避（テキドニセイリスベシ＝適当に・政治・宗教・スポーツ贔屓・学歴）' },
            ]},
            { category: '会話テクニック', terms: [
                { name: 'オープンクエスチョン', desc: '「はい/いいえ」で答えられない会話を広げる質問' },
                { name: 'バックトラッキング', desc: '相手の言葉を繰り返して共感を示す技法' },
                { name: 'パラフレーズ', desc: '相手の話を自分の言葉で言い換える技法' },
                { name: 'ミラーリング', desc: '相手の仕草や姿勢を自然に真似て親近感を高める技法' },
                { name: '自己開示', desc: '自分の経験や感情を適度に共有して距離を縮める手法' },
            ]},
        ];
        const colors = { '雑談の基本': '#f97316', '話題・ネタ': '#0891b2', '会話テクニック': '#7c3aed' };
        let html = `<div class="fade-in"><h1 style="font-size:1.6rem;margin-bottom:8px;">雑談用語リファレンス</h1>
            <p style="color:var(--text-secondary);margin-bottom:24px;">雑談で使われる重要用語の一覧です。</p>
